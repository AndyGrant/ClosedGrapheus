...
struct Quant : public Layer {
    Layer*            prev;
    GradientOperation grad_op;
    int               use_id;
    float             scalar;
    explicit Quant(Layer* prev, float scalar = 1)
        : Layer(prev->size)
        , prev(prev)
        , scalar(scalar) {
        prev->use();
    ...
    void forward() override {
        Layer::forward();
        operations::quant<data::GPU>(prev->dense_output.values, dense_output.values, scalar);
    }
    void backward() override {
        Layer::backward();
        operations::quant_bp<data::GPU>(prev->dense_output.values,
                                          prev->dense_output.gradients,
                                          dense_output.values,
                                          dense_output.gradients,
                                          scalar,
